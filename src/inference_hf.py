...
from .configs import HfGenerationConfig, HfInferenceConfig
...
if __name__ == "__main__":
    parser = TrlParser((HfInferenceConfig, ModelConfig, HfGenerationConfig))  # type: ignore
    configs: tuple[HfInferenceConfig, ModelConfig, HfGenerationConfig] = parser.parse_args_and_config()  # type: ignore
    ...
    logger = logging.getLogger("inference_hf")
    ...
    model = AutoModelForCausalLM.from_pretrained(
        pretrained_model_name_or_path=script_config.model_path or script_config.model,
    ...
    if accelerator.is_main_process:
        ...
        qa = qa.add_column("model_answer_ids", output_ids)  # type: ignore
        qa = qa.map(decode, fn_kwargs={"tokenizer": tokenizer, "feature": "model_answer"}, num_proc=script_config.num_proc)

        logger.info(f"model_answer: {qa[0]['model_answer']}")

        # Save to disk
        qa = qa.remove_columns(["messages", "prompt", "prompt_tokens", "model_answer_ids"])
